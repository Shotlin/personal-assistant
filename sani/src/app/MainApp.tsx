...
export default function MainApp() {
  ...
  useEffect(() => { const cleanups: Array<() => void> = []; let mounted = true; const start = async () => { cleanups.push(await onState(setState), await onPartial(setPartial), await onHistoryLoaded(setMessages), await onMessage(message => setMessages(old => [...old, message])), await onConversationChanged(async id => { const restored = await getMessages(id); if (mounted) setMessages(restored); })); const current = await getState(); if (mounted) { setState(current.state); setPartial(current.partial); } if (!ready.current) { ready.current = true; await mainReady(); } }; void start(); return () => { mounted = false; cleanups.forEach(cleanup => cleanup()); }; }, []);
  return <main className="main-app-shell"><MainSidebar selected={section} onSelect={setSection}/>{section === "home" ? <MainConversation messages={messages} state={state} partial={partial}/> : section === "layout" ? <OverlayLayoutEditor/> : <section className="section-boundary"><h1>{section === "control" ? "Computer Control" : section[0].toUpperCase()+section.slice(1)}</h1><p>This section is part of Sani’s desktop workspace and will gain its dedicated controls in a later approved phase.</p></section>}</main>;
}
