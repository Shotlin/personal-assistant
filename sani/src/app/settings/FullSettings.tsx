...
export default function FullSettings({ onOpenLayout, initialCategory = "General" }: { onOpenLayout: () => void; initialCategory?: Category }) {
  const { snapshot, microphones, agents, agentsAvailable, voiceModels, voiceProgress, error, saveGeneral, saveAi, saveProviderKey, selectAgent, installVoice, useVoice, removeVoice } = useSettings();
  ...
  return <section className="full-settings" aria-label="Full Settings">
    ...
    <div className="full-settings-grid"><nav aria-label="Settings categories">{CATEGORIES.map((item) => <button key={item} className={category === item ? "selected" : ""} onClick={() => setCategory(item)}>{item}</button>)}</nav><div className="settings-content">
      ...
      {category === "General" && <><h2>General</h2><label>Theme<select value={snapshot.theme} onChange={(e) => void saveGeneral({ theme: e.target.value })}><option value="dark">Dark</option><option value="light">Light</option></select></label><label>Next-turn agent<select value={snapshot.agent_mode} disabled={!agentsAvailable} onChange={(e) => void selectAgent(e.target.value)}>{agents.map((agent) => <option key={agent.id} value={agent.id}>{agent.name}</option>)}</select></label></>}
