...
export default function ComputerControlPage() {
  ...
  useEffect(refresh, []);
  return <section className="computer-control-page"><header><div><h1>Computer Control</h1><p>Permissions and runtime availability are checked locally each time you refresh.</p></div><button onClick={refresh}>Refresh</button></header><div className="settings-card"><strong>{snapshot?.status.replaceAll("_", " ") ?? "Checking…"}</strong><p>{snapshot?.message ?? "Checking current macOS permissions and Sani runtime…"}</p><dl><div><dt>Accessibility</dt><dd>{snapshot?.accessibility ?? "checking"}</dd></div><div><dt>Screen Recording</dt><dd>{snapshot?.screen_recording ?? "checking"}</dd></div><div><dt>Runtime</dt><dd>{snapshot?.runtime ?? "checking"}</dd></div></dl><div className="settings-inline"><button onClick={() => void invoke("request_accessibility").then(refresh)}>Request Accessibility</button><button onClick={() => void invoke("request_screen_recording").then(refresh)}>Request Screen Recording</button>{snapshot?.restart_required && <button onClick={() => void invoke("restart_app")}>Restart Sani</button>}</div></div></section>;
}
